...
- No hardcoded English anywhere a user can read it: handlers use `views.T` for every flash and inline message (login/register errors included); tab labels and chip labels go through `T` too. Accordions (`<details>`) are for optional bulk only (chunk lists); settings live in plain cards.
