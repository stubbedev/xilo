...
- Layout grids are `grid gap-6` for cards and `grid gap-4 sm:grid-cols-2 lg:grid-cols-4` for KPI tiles; `.grid > *` has `min-width: 0` globally (input.css) so long `<pre>`/tables shrink instead of widening the page. Sortable tables are `table-fixed` with sized meta columns and rely on `TableCard`'s min-width floor.
