...
func TestSmokeAllComponents(t *testing.T) {
	...
	cases := []struct {
		...
		{"Layout", views.Layout("Overview", true, views.Flash{}), "Overview"},
		{"Layout-flash-code", views.Layout("t", false, views.Flash{Msg: "created", Code: "secret123"}), "secret123"},
		...
		{"Settings", views.Settings(views.SettingsData{User: &store.User{Name: "admin", Role: "admin"}}), "Settings"},
		{"Settings-totp-passkeys", views.Settings(views.SettingsData{User: &store.User{Name: "admin", Role: "admin"}, TOTPEnabled: true, Passkeys: []store.Passkey{{ID: 1, Name: "yubikey", Created: 1}}, Users: []store.User{{ID: 1, Name: "admin", Role: "admin"}}, Orgs: []views.OrgInfo{{Account: store.Account{ID: 1, Slug: "default"}, Members: []store.AccountMember{{UserID: 1, UserName: "admin", Role: "admin"}}}}, MultiTenant: true, Plans: []store.Plan{{ID: 1, Name: "free", MaxCaches: 3, Public: true}}, Flash: views.Flash{Msg: "saved"}}), "yubikey"},
		{"PwHint-empty", views.PwHint(""), ""},
		{"PwHint-short", views.PwHint("short"), "pw-hint"},
		{"PwHint-weak", views.PwHint("weak"), "pw-hint"},
		{"PwHint-strong", views.PwHint("strong"), "pw-hint"},
		{"PwHint-mismatch", views.PwHint("mismatch"), "pw-hint"},
		{"TOTPEnroll", views.TOTPEnroll("data:image/png;base64,x", "SECRET"), "SECRET"},
		{"TOTPEnrollErr", views.TOTPEnrollErr("data:image/png;base64,x", "SECRET", "bad code"), "bad code"},
		{"Modal", views.Modal("m1", "Title"), "Title"},
		{"SectionHead", views.SectionHead("Head"), "Head"},
		{"SearchBox", views.SearchBox("/admin", "q", "val", "find...", "#t"), "find..."},
		{"HeadSearch", views.HeadSearch("q", "v", "search paths", "#t", "dlg1"), "search paths"},
		{"KPITile", views.KPITile("box", "42", "things", "kpi1"), "42"},
		{"KPITile-nclass", views.KPITile("box", "9", "over", "kpi2", "over"), "over"},
		{"Empty", views.Empty("inbox", "nothing here"), "nothing here"},
		{"IconBadge", views.IconBadge("ok", "tooltip", "check"), "tooltip"},
		{"ModalFoot", views.ModalFoot("check", "Save"), "Save"},
		{"ActionRow", views.ActionRow("hint text", "/admin/gc", "confirm.gc", "danger", "trash", "Delete", "extra"), "Delete"},
		{"NotFound-in", views.NotFound(true), "not found"},
		{"NotFound-out", views.NotFound(false), "not found"},
		{"Icon", views.Icon("check"), "svg"},
		{"IconClass", views.IconClass("sun", 20, "spin"), "svg"},
		{"CopyButton", views.CopyButton("copy-me"), "copy-me"},
		{"PagerNav", views.PagerNav(pager), "Next"},
		{"PagerNav-single", views.PagerNav(views.Pager{Page: 1, Pages: 1}), ""},
		{"SortTH-active", views.SortTH(sc, "name", "Name"), "Name"},
		{"SortTH-inactive", views.SortTH(sc, "size", "Size", "num"), "Size"},
		{"SortTH-desc", views.SortTH(views.SortCtx{Path: "/a", SortParam: "sort", DirParam: "dir", PageParam: "page", Key: "name", Dir: "desc"}, "name", "Name"), "descending"},
	}
