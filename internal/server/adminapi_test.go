...
func TestAdminAPI(t *testing.T) {
	...
	adminSecret, _, err := db.CreateToken("boss", nil, []string{"admin"}, 0)
	if err != nil {
		t.Fatal(err)
	}
	pushSecret, _, err := db.CreateToken("pleb", nil, []string{"push", "pull"}, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Run("cache lifecycle", func(t *testing.T) {
		...
		prio := 30
		var ret int64 = 3600
		resp, body = apiReq(t, ts, http.MethodPatch, "/api/v1/caches/apicache", adminSecret,
		...
		old := c.PubKey
		resp, body = apiReq(t, ts, http.MethodPost, "/api/v1/caches/apicache/rotate", adminSecret, nil)
		json.Unmarshal(body, &c)
		if resp.StatusCode != 200 || c.PubKey == old || c.PubKey == "" {
			t.Fatalf("rotate: %d %+v", resp.StatusCode, c)
		}

		resp, body = apiReq(t, ts, http.MethodGet, "/api/v1/caches/apicache", adminSecret, nil)
		...
		resp, _ = apiReq(t, ts, http.MethodDelete, "/api/v1/caches/apicache", adminSecret, nil)
		if resp.StatusCode != http.StatusNoContent {
			t.Fatalf("delete: %d", resp.StatusCode)
		}
		resp, _ = apiReq(t, ts, http.MethodGet, "/api/v1/caches/apicache", adminSecret, nil)
	...
	t.Run("token lifecycle", func(t *testing.T) {
		...
		if db.Authorize(ct.Secret, "any", "push", 0) {
	...
	t.Run("revoked admin token loses access", func(t *testing.T) {
		sec, tok, err := db.CreateToken("shortlived", nil, []string{"admin"}, 0)
		if err != nil {
			t.Fatal(err)
		}
