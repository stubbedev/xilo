...
// end to end with correctness assertions (not just status codes). Runs against
// the perf config; the auth matrix creates private caches + scoped tokens
// through the admin dashboard, so open-bootstrap-off behavior is covered too.
//
//   docker compose -f tests/k6/compose.yaml run --rm k6 run /scripts/ops.js
//
// Everything lives under the "default" account (/c/default/{cache}/…), the
// single-tenant convention. Multi-tenant semantics (registration, orgs, plans,
// quotas, isolation) are proven separately in mt.js.
//
// Threshold: every check must pass (rate==1). Overlap with perf.js/churn.js is
// deliberate — this suite proves semantics, those prove load behavior.
...
// The account every cache in this suite lives under.
const NS = ACCOUNT;
...
// c returns the /c/{account}/{cache} prefix for a cache in the test account.
function c(cache) {
  return `${BASE}/c/${NS}/${cache}`;
}

function putChunk(cache, data, headers, expect) {
  const hex = crypto.sha256(data, "hex");
  const res = http.put(`${c(cache)}/api/chunk/${hex}`, data, { headers });
...
// the returned HTML (shown exactly once). The cache scope is "account/cache";
// the active dashboard context must be that account (setContext handles it).
function createToken(name, perms, cacheScope) {
  const body = { name: name, cache: `${NS}/${cacheScope}` };
...
export default function () {
  ...
  // Idempotent across reruns of a persisted stack.
  const okOrFlash = (r) => r.status === 200;
  res = http.post(`${BASE}/admin/caches`, { name: "pub", namespace: NS, priority: "40" });
  must(res, "create cache pub", okOrFlash);
  res = http.post(`${BASE}/admin/caches`, { name: "priv", namespace: NS, priority: "40", private: "on" });
  must(res, "create cache priv", okOrFlash);

  res = http.get(`${BASE}/admin/cache/${NS}/pub`);
  ...
  res = http.get(`${BASE}/admin/account`);
  must(res, "account page", (r) => r.status === 200);
  ...
  const pushTok = createToken("k6-push", ["push", "pull"], "pub");
  const pullTok = createToken("k6-pull", ["pull"], "pub");
  ...
  res = http.get(`${c("pub")}/nix-cache-info`);
  ...
  res = http.put(`${c("pub")}/api/chunk/${h0}`, c0);
  must(res, "chunk push unauthenticated 401", (r) => r.status === 401);
  // pull-only token cannot push
  res = http.put(`${c("pub")}/api/chunk/${h0}`, c0, { headers: bearer(pullTok) });
  must(res, "chunk push with pull token 401", (r) => r.status === 401);
  // revoked token cannot push
  res = http.put(`${c("pub")}/api/chunk/${h0}`, c0, { headers: bearer(deadTok) });
  must(res, "chunk push with revoked token 401", (r) => r.status === 401);

  // config endpoint (advertises chunking params to pushers)
  res = http.get(`${c("pub")}/api/config`, { headers: bearer(pushTok) });
  must(res, "api config", (r) => {
    const cfg = JSON.parse(r.body);
    return r.status === 200 && cfg.minSize > 0 && cfg.avgSize >= cfg.minSize && cfg.maxSize >= cfg.avgSize &&
      cfg.publicKey.startsWith("pub:") && cfg.public === true;
  });

  // get-missing-chunks before upload: both missing
  res = http.post(`${c("pub")}/api/get-missing-chunks`, JSON.stringify({ hashes: [h0, h1] }),
  ...
  res = http.put(`${c("pub")}/api/chunk/${h0}`, c1, { headers: bearer(pushTok) });
  ...
  res = http.put(`${c("pub")}/api/path`, "{not json",
    { headers: Object.assign({ "Content-Type": "application/json" }, bearer(pushTok)) });
  ...
  res = http.post(`${c("pub")}/api/get-missing-paths`,
  ...
  res = http.get(`${c("pub")}/ffffffffffffffffffffffffffffffff.narinfo`);
  ...
  res = http.get(`${c("pub")}/nar/${storeHash}.nar`, {
  ...
  res = http.get(`${c("pub")}/nar/ffffffffffffffffffffffffffffffff.nar`);
  ...
  putChunk("priv", pc, bearer(privTok), 200);
  const privHash = putPath("priv", pseed, [ph], pnar, 4096, bearer(privTok), 200, "priv put-path 200");

  const privReads = [
    ["nix-cache-info", `${c("priv")}/nix-cache-info`],
    ["narinfo", `${c("priv")}/${privHash}.narinfo`],
    ["nar", `${c("priv")}/nar/${privHash}.nar`],
    ["api config", `${c("priv")}/api/config`],
  ];
  for (const [name, url] of privReads) {
    res = http.get(url);
    must(res, `priv ${name} anonymous 401`, (r) => r.status === 401);
    res = http.get(url, { headers: bearer(privTok) });
  ...
  const keyBefore = String(http.get(`${c("pub")}/api/config`, { headers: bearer(pushTok) }).body);
  res = http.post(`${BASE}/admin/cache/${NS}/pub/rotate`);
  must(res, "rotate key", (r) => r.status === 200);
  const keyAfter = String(http.get(`${c("pub")}/api/config`, { headers: bearer(pushTok) }).body);
  ...
  res = http.post(`${BASE}/admin/caches`, { name: "doomed", namespace: NS, priority: "40" });
  must(res, "create doomed cache", (r) => r.status === 200);
  res = http.post(`${BASE}/admin/cache/${NS}/doomed/delete`);
  must(res, "delete cache", (r) => r.status === 200);
  res = http.get(`${c("doomed")}/nix-cache-info`);
  ...
  res = http.get(`${BASE}/admin/cache/${NS}/pub?paths%5Bnumber%5D=2&paths%5Bsize%5D=1&paths%5Bq%5D=k6`);
  must(res, "cache detail paged path search", (r) => r.status === 200);
  res = http.get(`${BASE}/admin/cache/${NS}/nonexistent`);
  ...
  res = http.get(`${BASE}/static/xilo-tw.css`);
  ...
  res = http.post(`${BASE}/admin/tokens/${editId}/edit`,
    { name: "k6-push-renamed", cache: `${NS}/pub`, push: "on", pull: "on", permanent: "on" },
    { redirects: 0 });
  ...
  res = http.post(`${BASE}/admin/account/password/check`, { new: "swordfish-9", confirm: "swordfish-9" });
  must(res, "password strength check", (r) => r.status === 200);
  res = http.post(`${BASE}/admin/account/password`,
  ...
  // ---------- account email ----------
  res = http.post(`${BASE}/admin/account/email`, { email: "ops@example.com" });
  must(res, "set account email", (r) => r.status === 200);

  // ---------- TOTP 2FA full cycle ----------
  res = http.post(`${BASE}/admin/account/totp/enroll`);
  must(res, "totp enroll shows secret", (r) => r.status === 200 && /<code[^>]*>[A-Z2-7]{16,}<\/code>/.test(r.body));
  const secret = String(res.body).match(/<code[^>]*>([A-Z2-7]{16,})<\/code>/)[1];
  res = http.post(`${BASE}/admin/account/totp/enable`, { code: "000000" });
  must(res, "totp enable wrong code rejected", (r) => String(r.body).includes("match")); // "didn't match" (apostrophe HTML-escaped)
  res = http.post(`${BASE}/admin/account/totp/enable`, { code: totpCode(secret) });
